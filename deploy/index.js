...
function walkFiles(dir, fileList) {
  ...
  return _reduce(files, function(fileList, file) {
...
if (process.argv.length === 3) {
  ...
  async.series([
    _partial(upload, bucket, 'dist/offer-viewer.min.js', bundle, 'application/javascript', 'gzip'),
    _partial(uploadFile, bucket, 'index.html'),
    _partial(uploadFiles, bucket, 'assets/')
  ]);
} else {
